export const fr = {
  // ── Auth ──
  'auth.login.title': 'Connectez-vous à votre instance',
  'auth.login.email': 'Email',
  'auth.login.password': 'Mot de passe',
  'auth.login.submit': 'Se connecter',
  'auth.login.error': 'Connexion impossible.',
  'auth.login.noAccount': 'Pas encore de compte ?',
  'auth.login.createAccount': 'Créer un compte',
  'auth.keycloak': 'Continuer avec Keycloak',
  'auth.register.title': 'Créez votre compte administrateur',
  'auth.register.name': 'Nom',
  'auth.register.email': 'Email',
  'auth.register.password': 'Mot de passe',
  'auth.register.passwordHint': '8 caractères minimum',
  'auth.register.submit': 'Créer mon compte',
  'auth.register.error': 'Impossible de créer le compte.',
  'auth.register.hasAccount': 'Déjà un compte ?',
  'auth.register.login': 'Se connecter',
  'auth.logout': 'Se déconnecter',

  // ── Password management ──
  'auth.changePassword.title': 'Changer le mot de passe',
  'auth.changePassword.current': 'Mot de passe actuel',
  'auth.changePassword.new': 'Nouveau mot de passe',
  'auth.changePassword.confirm': 'Confirmer le nouveau mot de passe',
  'auth.changePassword.submit': 'Modifier',
  'auth.changePassword.success': 'Mot de passe modifié',
  'auth.changePassword.errorWrong': 'Mot de passe actuel incorrect',
  'auth.changePassword.errorMismatch': 'Les mots de passe ne correspondent pas',
  'auth.changePassword.passwordHint': '8 caractères minimum',
  'auth.forgot.link': 'Mot de passe oublié ?',
  'auth.forgot.title': 'Réinitialiser le mot de passe',
  'auth.forgot.email': 'Email',
  'auth.forgot.submit': 'Envoyer le lien',
  'auth.forgot.success': 'Si un compte existe avec cet email, un lien de réinitialisation a été envoyé.',
  'auth.forgot.back': 'Retour à la connexion',
  'auth.reset.title': 'Nouveau mot de passe',
  'auth.reset.password': 'Mot de passe',
  'auth.reset.confirm': 'Confirmer le mot de passe',
  'auth.reset.submit': 'Réinitialiser',
  'auth.reset.success': 'Mot de passe réinitialisé. Vous pouvez vous connecter.',
  'auth.reset.expired': 'Ce lien a expiré ou est invalide.',
  'auth.reset.tryAgain': 'Demander un nouveau lien',
  'password.strength.weak': 'Faible',
  'password.strength.fair': 'Correct',
  'password.strength.strong': 'Fort',

  // ── Navigation ──
  'nav.overview': 'Project Overview',
  'nav.tables': 'Tables',
  'nav.sqlEditor': 'Éditeur SQL',

  // ── Overview ──
  'overview.noConnection': 'Aucune connexion sélectionnée',
  'overview.noConnectionHint': 'Choisissez une connexion dans le menu en haut à gauche',
  'overview.stats': 'Base de données',
  'overview.tables': 'Tables',
  'overview.views': 'Vues',
  'overview.functions': 'Fonctions',
  'overview.dbSize': 'Taille',
  'overview.health': 'Santé de la connexion',
  'overview.connected': 'Connecté',
  'overview.disconnected': 'Déconnecté',
  'overview.latency': 'Latence',
  'overview.latencyTooltip': 'Tester la connexion et mesurer la latence',
  'overview.version': 'Version serveur',
  'overview.encoding': 'Encodage',
  'overview.timezone': 'Fuseau horaire',
  'overview.recentQueries': 'Requêtes récentes',
  'overview.recentSaved': 'Requêtes sauvegardées récentes',
  'overview.topTables': 'Tables les plus visitées',
  'overview.pinned': 'Requêtes épinglées',
  'overview.noHistory': 'Aucune requête récente',
  'overview.noPinned': 'Aucune requête épinglée — cliquer ★ sur une requête',
  'overview.noTopTables': 'Aucune visite de table',
  'overview.erd': 'Modèle de données',
  'overview.erdHint': 'Scroll pour zoomer · glisser pour naviguer · cliquer pour ouvrir',
  'overview.noSchema': 'Aucune table trouvée',
  'overview.noSaved': 'Aucune requête sauvegardée',
  'overview.pin': 'Épingler',
  'overview.unpin': 'Désépingler',
  'overview.showMore': '+ {count} de plus…',
  'overview.erdShowMore': 'Voir les {count} autres tables',
  'overview.queryHistory': 'Historique des requêtes exécutées',
  'overview.dbUsers': 'Utilisateurs',
  'overview.dbUsersTooltip': 'Gérer les utilisateurs et les privilèges de la base',

  // ── Connections ──
  'conn.none': 'Aucune connexion',
  'conn.selectOrCreate': 'Sélectionnez ou créez',
  'conn.search': 'Rechercher...',
  'conn.label': 'Connexions',
  'conn.new': 'Nouvelle connexion',
  'conn.edit': 'Modifier la connexion',
  'conn.delete': 'Supprimer la connexion',
  'conn.deleteConfirm': 'Les requêtes sauvegardées liées à cette connexion perdront leur association.',
  'conn.deleteAction': 'Supprimer',
  'conn.titleNew': 'Nouvelle connexion',
  'conn.titleEdit': 'Modifier la connexion',
  'conn.tabString': 'Connection string',
  'conn.tabManual': 'Champs manuels',
  'conn.pasteString': 'Collez votre connection string',
  'conn.parseError': 'Format non reconnu. Attendu : postgresql://user:pass@host:port/db ou mongodb+srv://user:pass@cluster/db',
  'conn.parsedDriver': 'Driver',
  'conn.parsedHost': 'Hôte',
  'conn.parsedDb': 'Base',
  'conn.parsedUser': 'User',
  'conn.parsedSsl': 'SSL',
  'conn.parsedSslYes': 'oui',
  'conn.parsedSslNo': 'non',
  'conn.name': 'Nom',
  'conn.namePlaceholder': 'Ma base prod',
  'conn.environment': 'Environnement',
  'conn.environmentOptional': '(optionnel)',
  'conn.host': 'Hôte',
  'conn.port': 'Port',
  'conn.database': 'Base de données',
  'conn.databaseHint': '(optionnel — serveur entier)',
  'conn.databasePlaceholder': 'Laisser vide pour tout le serveur',
  'conn.catalog': 'Catalogue',
  'conn.catalogHint': '(catalogue ou catalogue/schéma — requis pour l’explorateur de schéma)',
  'conn.catalogPlaceholder': 'hive ou hive/default',
  'conn.hostOrUri': 'Hôte ou URI',
  'conn.hostOrUriPlaceholder': 'localhost ou mongodb+srv://cluster0.exemple.net',
  'conn.mongoDatabaseHint': '(optionnel — « test » par défaut)',
  'conn.mongoDatabasePlaceholder': 'test',
  'conn.redisHostPlaceholder': 'localhost ou redis://:motdepasse@hôte:6379/0',
  'conn.redisDb': 'Index de base',
  'conn.redisDbHint': '(optionnel — 0 par défaut)',
  'conn.mssqlHostPlaceholder': 'serveur ou serveur\\INSTANCE',
  'conn.account': 'Compte',
  'conn.accountPlaceholder': 'monorg-moncompte ou son URL',
  'conn.snowflakeDatabaseHint': '(base ou base/schéma — requis pour l’explorateur de schéma)',
  'conn.snowflakeDatabasePlaceholder': 'ANALYTICS ou ANALYTICS/PUBLIC',
  'conn.warehouse': 'Warehouse',
  'conn.role': 'Rôle',
  'conn.passwordOrKey': 'Mot de passe ou clé privée',
  'conn.passwordOrKeyPlaceholder': 'Mot de passe, ou clé PEM (-----BEGIN PRIVATE KEY-----…)',
  'conn.username': 'Utilisateur',
  'conn.password': 'Mot de passe',
  'conn.passwordUnchanged': '(inchangé)',
  'conn.filePath': 'Chemin du fichier',
  'conn.filePathPlaceholder': '/données/app.db ou :memory:',
  'conn.test': 'Tester',
  'conn.copyString': 'Copier la chaîne',
  'conn.copied': 'Copié',
  'conn.testFail': 'Échec de connexion',
  'conn.cancel': 'Annuler',
  'conn.save': 'Enregistrer',
  'conn.create': 'Créer',

  // ── Tabs ──
  'tab.new': 'Nouvelle requête',
  'tab.newTooltip': 'Nouvelle requête · Alt+N',
  'tab.close': 'Fermer',
  'tab.closeOthers': 'Fermer les autres',
  'tab.closeLeft': 'Fermer à gauche',
  'tab.closeRight': 'Fermer à droite',
  'tab.closeAll': 'Fermer tous',

  // ── Editor ──
  'editor.cut': 'Couper',
  'editor.copy': 'Copier',
  'editor.paste': 'Coller',
  'editor.selectAll': 'Sélectionner tout',
  'editor.beautify': 'Formater le SQL',
  'editor.save': 'Sauvegarder',
  'editor.saveTooltip': 'Sauvegarder · Ctrl+S',
  'editor.saveAs': 'Enregistrer sous…',
  'editor.saveAsTooltip': 'Ctrl+Shift+S',
  'editor.execute': 'Exécuter',
  'editor.executeSelection': 'Exécuter la sélection',
  'editor.executeTooltip': 'Ctrl+Enter',
  'editor.selectConnection': 'Sélectionnez une connexion',
  'editor.resultsHere': 'Les résultats apparaîtront ici',
  'editor.ctrlEnter': 'Ctrl+Enter pour exécuter',
  'editor.running': 'Exécution en cours...',
  'editor.rowsReceived': 'lignes reçues...',
  'editor.createOrExplore': 'Créez une requête ou explorez une table',

  // ── Results ──
  'results.lines': 'ligne',
  'results.lines_plural': 'lignes',
  'results.affected': 'affectée',
  'results.affected_plural': 'affectées',
  'results.success': 'Requête exécutée avec succès',
  'results.error': 'Erreur inconnue',
  'results.explain': 'Expliquer',
  'results.perPage': 'Lignes par page',
  'results.all': 'Tout',
  'results.statement': 'Requête {n}',

  // ── Table mode ──
  'table.filter': 'Filtrer',
  'table.sort': 'Trier',
  'table.insert': 'Insérer',
  'table.insertRow': 'Nouvelle ligne',
  'table.insertColumn': 'Nouvelle colonne',
  'table.importCsv': 'Import rapide (CSV)',
  'table.addFilter': 'Ajouter un filtre',
  'table.removeFilters': 'Supprimer les filtres',
  'table.applyFilters': 'Appliquer les filtres',
  'table.addSort': 'Ajouter une colonne de tri',
  'table.removeSorts': 'Supprimer les tris',
  'table.applySort': 'Appliquer le tri',
  'table.sortBy': 'trier par',
  'table.thenBy': 'puis par',
  'table.ascending': 'croissant :',
  'table.filterValue': 'Saisir une valeur',
  'table.noResults': 'Aucun résultat.',

  // ── Selection ──
  'sel.delete': 'Supprimer',
  'sel.deleteLine': 'ligne',
  'sel.deleteLines': 'lignes',
  'sel.copy': 'Copier',
  'sel.copyAsCsv': 'Copier en CSV',
  'sel.copyAsJson': 'Copier en JSON',
  'sel.copyAsSql': 'Copier en SQL',
  'sel.copyAsMongo': 'Copier en mongosh',
  'sel.copyAsRedis': 'Copier en redis-cli',
  'sel.export': 'Exporter',
  'sel.exportAsCsv': 'Exporter en CSV',
  'sel.exportAsJson': 'Exporter en JSON',
  'sel.exportAsSql': 'Exporter en SQL',
  'sel.exportAsMongo': 'Exporter en script mongosh',
  'sel.exportAsRedis': 'Exporter en commandes redis-cli',
  'table.noPrimaryKey': 'Aucune clé primaire connue pour cette table : modification impossible.',

  // ── Context menu ──
  'ctx.copyCell': 'Copier la cellule',
  'ctx.copyCells': 'Copier {rows}x{cols} cellules',
  'ctx.copyRow': 'Copier la ligne',
  'ctx.copyRows': 'Copier {count} lignes',
  'ctx.addAsFilter': 'Ajouter comme filtre',
  'ctx.editRecord': 'Modifier l\'enregistrement',
  'ctx.deleteRecord': 'Supprimer',
  'ctx.deleteRecords': 'Supprimer {count} lignes',
  'ctx.deleteRecordTitle': 'Supprimer l\'enregistrement',
  'ctx.deleteRecordConfirm': 'Êtes-vous sûr de vouloir supprimer cet enregistrement ? Cette action est irréversible.',
  'ctx.deleteRecordsConfirm': 'Vous allez supprimer {count} lignes. Cette action est irréversible.',
  'ctx.slideToDelete': 'Glisser pour supprimer {count} lignes',

  // ── Sheet (insert/edit) ──
  'sheet.newRow': 'Nouvelle ligne',
  'sheet.newColumn': 'Nouvelle colonne',
  'sheet.importCsv': 'Import rapide (CSV)',
  'sheet.editRecord': 'Modifier l\'enregistrement',
  'sheet.columnName': 'Nom de la colonne',
  'sheet.columnType': 'Type',
  'sheet.csvHint': 'Données CSV (première ligne = en-têtes)',
  'sheet.cancel': 'Annuler',
  'sheet.insertRow': 'Insérer la ligne',
  'sheet.save': 'Enregistrer',
  'sheet.addColumn': 'Ajouter la colonne',
  'sheet.import': 'Importer',
  'sheet.chooseDate': 'Choisir une date',

  // ── Saved queries ──
  'sq.title': 'Requêtes sauvegardées',
  'sq.none': 'Aucune requête sauvegardée.',
  'sq.search': 'Filtrer les requêtes...',
  'sq.open': 'Ouvrir',
  'sq.rename': 'Renommer',
  'sq.duplicate': 'Dupliquer',
  'sq.share': 'Partager',
  'sq.shared': 'Partagées',
  'sq.shareTitle': 'Partager la requête',
  'sq.delete': 'Supprimer',
  'sq.moveTo': 'Déplacer vers',
  'sq.removeFromFolder': 'Retirer du dossier',
  'sq.newFolder': 'Nouveau dossier…',
  'sq.newFolderTitle': 'Nouveau dossier',
  'sq.folderName': 'Nom du dossier',
  'sq.create': 'Créer',
  'sq.saveTitle': 'Sauvegarder la requête',
  'sq.saveName': 'Nom',
  'sq.saveNamePlaceholder': 'Ex: Active users count',
  'sq.saveDescription': 'Description',
  'sq.saveDescriptionOptional': '(optionnel)',
  'sq.saveDescriptionPlaceholder': 'Description courte...',
  'sq.saveAction': 'Sauvegarder',
  'sq.saved': 'Requête sauvegardée',
  'sq.timeline': 'Timeline',
  'sq.timeline.current': 'Actuel (éditeur)',
  'sq.timeline.editing': 'En cours de modification',
  'sq.timeline.comparing': 'Comparaison',
  'sq.timeline.addLabel': 'Ajouter un label',
  'sq.timeline.noVersions': 'Aucun historique de versions',
  'sq.timeline.vs': 'vs',
  'sq.timeline.sequential': 'vs version précédente',
  'sq.timeline.vsCurrent': 'vs actuel',
  'sq.timeline.labelSaved': 'Label enregistré',
  'sq.timeline.restore': 'Restaurer',
  'sq.timeline.restored': 'Version restaurée',
  'sq.collaborative': 'Collaboratif',
  'sq.collaborativeHint': 'Peut modifier et sauvegarder cette requête',
  'collab.connected': 'Connecté',
  'collab.disconnected': 'Déconnecté',
  'collab.reconnecting': 'Reconnexion...',
  'collab.participants': '{count} participant(s)',
  'chat.title': 'Chat',
  'chat.placeholder': 'Écrire un message...',
  'chat.send': 'Envoyer',
  'chat.noMessages': 'Aucun message',
  'chat.scrollToUser': 'Aller au curseur',

  // ── Copilot ──
  'copilot.title': 'Copilot',
  'copilot.subtitle': 'Claude',
  'copilot.tooltip': 'Copilot IA · Claude',
  'copilot.selectConnection': 'Sélectionnez une connexion pour utiliser le copilot',
  'copilot.placeholder': 'Demandez au copilot...',
  'copilot.emptyHint': 'Demandez-moi n\'importe quoi sur votre base de données. Je connais votre schéma.',
  'copilot.suggestion1': 'Les 10 derniers enregistrements',
  'copilot.suggestion2': 'Compter les lignes par table',
  'copilot.suggestion3': 'Expliquer cette requête',
  'copilot.suggestion4': 'Génère-moi un jeu de données réaliste',
  'copilot.insert': 'Insérer',
  'copilot.copySql': 'Copier le SQL',
  'copilot.insertToEditor': 'Insérer dans l\'éditeur',
  'copilot.errorContact': 'Impossible de contacter le copilot.',
  'copilot.subtitleOpenai': 'OpenAI',
  'copilot.subtitleAzure': 'Azure OpenAI',
  'copilot.subtitleMistral': 'Mistral',
  'copilot.regenerate': 'Régénérer',
  'copilot.clear': 'Effacer la conversation',
  'copilot.explainError.button': 'Expliquer avec Copilot',
  'copilot.explainError.prompt': 'J\'ai exécuté cette requête SQL et j\'obtiens une erreur. Aide-moi à comprendre et corriger.\n\n**Requête :**\n```sql\n{sql}\n```\n\n**Erreur :**\n{error}',

  // ── Dump ──
  'dump.table': 'Dumper la table',
  'dump.allTables': 'Dumper toutes les tables',
  'dump.structureOnly': 'Structure uniquement',
  'dump.structureAndData': 'Structure + Données',

  // ── Sync ──
  'sync.title': 'Synchronisation de données',
  'sync.tooltip': 'Synchroniser les données entre connexions',
  'sync.source': 'Source',
  'sync.target': 'Cible',
  'sync.selectConnection': 'Sélectionner une connexion',
  'sync.fetchingSchema': 'Récupération du schéma…',
  'sync.noTables': 'Aucune table trouvée sur la source.',
  'sync.tables': 'Tables à synchroniser',
  'sync.selectAll': 'Tout sélectionner',
  'sync.deselectAll': 'Tout désélectionner',
  'sync.start': 'Synchroniser',
  'sync.syncing': 'Synchronisation en cours…',
  'sync.done': 'Synchronisation terminée',
  'sync.error': 'Échec de la synchronisation',
  'sync.tableStatus.pending': 'En attente',
  'sync.tableStatus.schema': 'Lecture du schéma…',
  'sync.tableStatus.create': 'Création de la table…',
  'sync.tableStatus.insert': 'Insertion des données…',
  'sync.tableStatus.done': 'Terminé',
  'sync.tableStatus.error': 'Erreur',
  'sync.rows': 'lignes',
  'sync.modeStructure': 'Structure uniquement',
  'sync.modeStructureData': 'Structure + Données',
  'sync.includeConstraints': 'Inclure les index et clés étrangères',
  'sync.database': 'Base de données',
  'sync.warning': 'Les tables cibles seront supprimées et recréées. Cette action est irréversible.',

  // ── Command palette ──
  'cmd.placeholder': 'Rechercher une action, une table, une requête...',
  'cmd.noResults': 'Aucun résultat.',
  'cmd.actions': 'Actions',
  'cmd.navigation': 'Navigation',
  'cmd.connections': 'Connexions',
  'cmd.savedQueries': 'Requêtes sauvegardées',
  'cmd.session': 'Session',
  'cmd.newQuery': 'Nouvelle requête',
  'cmd.save': 'Sauvegarder',
  'cmd.saveAs': 'Enregistrer sous…',
  'cmd.execute': 'Exécuter la requête',
  'cmd.executeSelection': 'Exécuter la sélection',
  'cmd.refresh': 'Rafraîchir les résultats',
  'cmd.newConnection': 'Nouvelle connexion',
  'cmd.active': 'actif',

  // ── User menu ──
  'user.role.admin': 'Administrateur',
  'user.role.editor': 'Éditeur',
  'user.role.viewer': 'Lecteur',
  'user.language': 'Langue',
  'user.french': 'Français',
  'user.english': 'English',
  'user.theme': 'Thème',
  'user.theme.system': 'Système',
  'user.theme.dark': 'Sombre',
  'user.theme.light': 'Clair',

  // ── Common ──
  'common.cancel': 'Annuler',
  'common.delete': 'Supprimer',
  'common.loading': 'Chargement...',
  'common.init': 'Initialisation...',
  'common.noTableFound': 'Aucune table trouvée.',
  'common.schemaError': 'Impossible de charger le schéma.',
  'common.retry': 'Réessayer',
  'common.filterTables': 'Filtrer les tables...',
  'common.schema': 'Schema',
  // ── Admin ──
  'admin.title': 'Administration',
  'admin.users': 'Utilisateurs',
  'admin.connections': 'Connexions',
  'admin.noUsers': 'Aucun utilisateur.',
  'admin.editUser': 'Modifier l\'utilisateur',
  'admin.deleteUser': 'Supprimer l\'utilisateur',
  'admin.deleteUserConfirm': 'Supprimer cet utilisateur ? Ses requêtes sauvegardées et connexions personnelles seront supprimées.',
  'admin.cannotDeleteSelf': 'Vous ne pouvez pas supprimer votre propre compte.',
  'admin.role': 'Rôle',
  'admin.userSaved': 'Utilisateur mis à jour',
  'admin.userDeleted': 'Utilisateur supprimé',
  'admin.slideToDelete': 'Glisser pour supprimer',
  'admin.groups': 'Groupes',
  'admin.noGroups': 'Aucun groupe.',
  'admin.newGroup': 'Nouveau groupe',
  'admin.editGroup': 'Modifier le groupe',
  'admin.deleteGroup': 'Supprimer le groupe',
  'admin.deleteGroupConfirm': 'Supprimer ce groupe ? Les membres et accès aux connexions associés seront retirés.',
  'admin.groupName': 'Nom du groupe',
  'admin.groupDescription': 'Description',
  'admin.groupMembers': 'Membres',
  'admin.groupSaved': 'Groupe mis à jour',
  'admin.groupCreated': 'Groupe créé',
  'admin.groupDeleted': 'Groupe supprimé',
  'admin.addMembers': 'Ajouter des membres',
  'admin.addGroups': 'Ajouter des groupes',
  'admin.removeMember': 'Retirer du groupe',
  'admin.connGroups': 'Groupes',
  'admin.connPublic': 'Connexion publique',

  // ── Column header menu ──
  'col.pin': 'Ancrer la colonne',
  'col.unpin': 'Désancrer la colonne',
  'col.hide': 'Cacher la colonne',
  'col.fitWidth': 'Adapter la taille',
  'col.showHidden': '{count} colonne(s) cachée(s)',
  'col.showAll': 'Tout afficher',

  'fn.addParam': 'Ajouter un paramètre',
  'table.modifyStructure': 'Modifier la structure',
  'table.addColumn': 'Ajouter une colonne',
  'table.createIndex': 'Créer l\'index',
  'table.columnName': 'Nom',
  'table.columnType': 'Type',
  'table.columnNullable': 'Nullable',
  'table.columnDefault': 'Défaut',
  'table.deleteColumn': 'Supprimer la colonne',
  'table.slideToApply': 'Glisser pour appliquer',
  'table.tableNameRequired': 'Le nom de la table est obligatoire',
  'table.renameColumn': 'Renommer la colonne',
  'table.newTable': 'Nouvelle table',
  'table.tableName': 'Nom de la table',
  'table.comment': 'Commentaire',
  'table.size': 'Taille',
  'table.decimal': 'Déc.',
  'table.columns': 'Colonnes',
  'table.indexes': 'Index',
  'table.foreignKeys': 'Clés étrangères',
  'table.triggers': 'Triggers',
  'table.options': 'Options',
  'table.comingSoon': 'À venir',
  'common.dropTable': 'Supprimer la table',
  'common.dropView': 'Supprimer la vue',
  'common.dropTableConfirm': 'et toutes ses données ? Cette action est irréversible.',
  'common.dropViewConfirm': '? Cette action est irréversible.',

  // ── DB Users ──
  'dbusers.title': 'Utilisateurs de la base',
  'dbusers.createUser': 'Créer un utilisateur',
  'dbusers.identity': 'Identité',
  'dbusers.serverPrivileges': 'Privilèges serveur',
  'dbusers.tablePrivileges': 'Privilèges par table',
  'dbusers.advanced': 'Avancé',
  'dbusers.username': "Nom d'utilisateur",
  'dbusers.host': 'Hôte',
  'dbusers.password': 'Mot de passe',
  'dbusers.confirmPassword': 'Confirmer le mot de passe',
  'dbusers.passwordOptional': 'Laisser vide pour conserver le mot de passe actuel',
  'dbusers.passwordMismatch': 'Les mots de passe ne correspondent pas',
  'dbusers.save': 'Enregistrer',
  'dbusers.dropUser': "Supprimer l'utilisateur",
  'dbusers.slideToDropUser': 'Glisser pour supprimer',
  'dbusers.maxQueriesPerHour': 'Max requêtes / heure',
  'dbusers.maxUpdatesPerHour': 'Max mises à jour / heure',
  'dbusers.maxConnectionsPerHour': 'Max connexions / heure',
  'dbusers.maxUserConnections': 'Max connexions simultanées',
  'dbusers.connectionLimit': 'Limite de connexions (-1 = illimité)',
  'dbusers.profile': 'Profil Oracle',
  'dbusers.database': 'Base de données / Schéma',
  'dbusers.table': 'Table',
  'dbusers.addTablePrivilege': 'Ajouter une règle par table',
  'dbusers.removeTablePrivilege': 'Retirer',
  'dbusers.selectAll': 'Tout sélectionner',
  'dbusers.deselectAll': 'Tout désélectionner',
  'dbusers.noUserSelected': 'Sélectionner un utilisateur ou en créer un',
  'dbusers.created': 'Utilisateur créé',
  'dbusers.updated': 'Utilisateur mis à jour',
  'dbusers.dropped': 'Utilisateur supprimé',
  'dbusers.plugin': 'Plugin',
  'dbusers.status': 'Statut',
  'dbusers.noUsers': 'Aucun utilisateur',
  'dbusers.notSupported': 'Gestion des utilisateurs non supportée pour ce type de base.',

  // ── Data Import ──
  'import.title': 'Importer des données',
  'import.tooltip': 'Importer un fichier',
  'import.stepUpload': 'Fichier',
  'import.stepConfigure': 'Configuration',
  'import.stepImport': 'Import',
  'import.dropHint': 'Glissez un fichier ici ou cliquez pour parcourir',
  'import.unsupportedFormat': 'Format non supporté. Formats acceptés : CSV, TXT, JSON, XML, XLS, XLSX',
  'import.emptyFile': 'Le fichier est vide ou ne contient aucune donnée exploitable.',
  'import.changeFile': 'Changer',
  'import.delimiter': 'Délimiteur',
  'import.delimiterComma': 'Virgule (,)',
  'import.delimiterSemicolon': 'Point-virgule (;)',
  'import.delimiterTab': 'Tabulation',
  'import.delimiterPipe': 'Pipe (|)',
  'import.hasHeader': 'La première ligne contient les en-têtes',
  'import.sheet': 'Feuille',
  'import.tableName': 'Nom de la table',
  'import.ifExists': 'Si la table existe',
  'import.ifExistsError': 'Erreur',
  'import.ifExistsAppend': 'Ajouter les données',
  'import.ifExistsReplace': 'Remplacer la table',
  'import.columns': 'Colonnes',
  'import.preview': 'Aperçu',
  'import.rows': 'lignes',
  'import.next': 'Suivant',
  'import.back': 'Retour',
  'import.startImport': 'Importer',
  'import.importing': 'Import en cours…',
  'import.rowsInserted': 'lignes importées dans',
  'import.success': 'Import terminé avec succès',
  'import.error': 'Erreur lors de l\'import',
  'import.mapWithCopilot': 'Reconnaître avec Copilot',
  'import.unknownColumns': 'Certaines colonnes n\'existent pas dans la table cible',
  'import.unknownError': 'Une erreur inconnue est survenue.',
  'import.close': 'Fermer',
  'db.createTooltip': 'Nouvelle database',
  'db.createTitle': 'Créer une database',
  'db.createPlaceholder': 'nom_de_la_base',
  'db.createConfirm': 'Créer',
  'db.createSuccess': 'Database "{name}" créée',
  'db.createError': 'Erreur lors de la création',

  // ── Unsaved changes ──
  'unsaved.title': 'Modifications non sauvegardées',
  'unsaved.message': '« {name} » contient des modifications non sauvegardées. Voulez-vous les sauvegarder ?',
  'unsaved.save': 'Sauvegarder',
  'unsaved.dontSave': 'Ne pas sauvegarder',
  'unsaved.cancel': 'Annuler',
} as const
